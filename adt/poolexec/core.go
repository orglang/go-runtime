...
type API interface {
	...
	Retrieve(identity.ADT) (PoolSnap, error)
...
func (s *service) Take(spec StepSpec) (err error) {
	...
	for termSpec != nil {
		...
		var sigs map[identity.ADT]procdecl.ProcRec
		err = s.operator.Implicit(ctx, func(ds sd.Source) error {
		...
		typeQNs := procdecl.CollectEnv(maps.Values(sigs))
		var types map[qualsym.ADT]typedef.TypeRec
		err = s.operator.Implicit(ctx, func(ds sd.Source) error {
...
func (s *service) Retrieve(poolID identity.ADT) (snap PoolSnap, err error) {
	ctx := context.Background()
	err = s.operator.Implicit(ctx, func(ds sd.Source) error {
...
func convertToCtx(poolID identity.ADT, chnls []procexec.EP, types map[identity.ADT]typedef.TermRec) typedef.Context {
...
func (s *service) checkClient(
	...
	case procdef.SpawnSpecOld:
		...
		if !ok {
			err := procdecl.ErrRootMissingInEnv(termSpec.SigID)
			s.log.Error("checking failed")
			return err
		}
		// check vals
